...
#include "queryopenedenterprisedlpfilesproxy_fuzzer.h"
#include <iostream>
#include <string>
#include <vector>
...
static void FuzzTest(const uint8_t* data, size_t size)
{
    ...
    DlpPermissionClient::GetInstance().QueryOpenedEnterpriseDlpFiles(label, resultUris);
}

bool QueryOpenedEnterpriseDlpFilesProxyFuzzTest(const uint8_t* data, size_t size)
{
    FuzzTest(data, size);
    return true;
}
...
{
    OHOS::QueryOpenedEnterpriseDlpFilesProxyFuzzTest(data, size);
    return 0;
}
