...
#include "getdlpgatheringpolicystub_fuzzer.h"
#include <iostream>
#include <string>
#include <thread>
#include <vector>
...
static void FuzzTest(const uint8_t* data, size_t size)
{
    ...
    uint32_t code = static_cast<uint32_t>(IDlpPermissionServiceIpcCode::COMMAND_GET_DLP_GATHERING_POLICY);
    MessageParcel reply;
...
bool GetDlpGatheringPolicyFuzzTest(const uint8_t* data, size_t size)
{
    FuzzTest(data, size);
    return true;
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::GetDlpGatheringPolicyFuzzTest(data, size);
    return 0;
}
