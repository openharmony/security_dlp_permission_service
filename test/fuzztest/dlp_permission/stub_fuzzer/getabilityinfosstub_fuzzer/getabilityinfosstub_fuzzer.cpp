...
#include "getabilityinfosstub_fuzzer.h"
...
static const size_t MIN_DATA_SIZE = sizeof(int32_t) * 2;
} // namespace
...
{
    if ((data == nullptr) || (size < MIN_DATA_SIZE) || (size > MAX_SAFE_SIZE)) {
        return;
    }
    FuzzedDataProvider fdp(data, size);
    std::string bundleName = fdp.ConsumeBytesAsString(size / 3);
    ...
    uint32_t code = static_cast<uint32_t>(IDlpPermissionServiceIpcCode::COMMAND_GET_ABILITY_INFOS);
    MessageParcel reply;
...
bool GetAbilityInfosFuzzTest(const uint8_t* data, size_t size)
{
    FuzzTest(data, size);
    return true;
}
...
{
    OHOS::GetAbilityInfosFuzzTest(data, size);
    return 0;
}
