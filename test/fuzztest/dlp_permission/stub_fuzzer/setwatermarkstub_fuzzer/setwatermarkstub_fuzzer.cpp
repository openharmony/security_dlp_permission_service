...
#include "setwatermarkstub_fuzzer.h"
...
{
    if ((data == nullptr) || (size < sizeof(int32_t)) || (size > MAX_SAFE_SIZE)) {
        return;
    }
    ...
    uint32_t code = static_cast<uint32_t>(IDlpPermissionServiceIpcCode::COMMAND_SET_WATER_MARK);
    MessageParcel reply;
...
bool SetWaterMarkFuzzTest(const uint8_t* data, size_t size)
{
    FuzzTest(data, size);
    return true;
}
...
{
    OHOS::SetWaterMarkFuzzTest(data, size);
    return 0;
}
