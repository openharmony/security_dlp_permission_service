...
#include "setreadflagstub_fuzzer.h"
#include <iostream>
...
static void FuzzTest(const uint8_t* data, size_t size)
{
    ...
    uint32_t code = static_cast<uint32_t>(IDlpPermissionServiceIpcCode::COMMAND_SET_READ_FLAG);
    MessageParcel reply;
...
bool SetReadFlagFuzzTest(const uint8_t* data, size_t size)
{
    FuzzTest(data, size);
    return true;
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::SetReadFlagFuzzTest(data, size);
    return 0;
}
