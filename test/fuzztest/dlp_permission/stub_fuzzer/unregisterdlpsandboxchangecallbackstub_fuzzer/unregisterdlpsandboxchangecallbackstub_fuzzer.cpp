...
#include "unregisterdlpsandboxchangecallbackstub_fuzzer.h"
#include <iostream>
#include <string>
#include <thread>
#include <vector>
...
static void FuzzTest(const uint8_t* data, size_t size)
{
    ...
    uint32_t code = static_cast<uint32_t>(DlpPermissionServiceInterfaceCode::UNREGISTER_DLP_SANDBOX_CHANGE_CALLBACK);
    MessageParcel reply;
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::UnRegisterDlpSandboxChangeCallbackFuzzTest(data, size);
    return 0;
}
