...
#include "setenterprisepolicystub_fuzzer.h"
...
static void FuzzTest(const uint8_t* data, size_t size)
{
    ...
    if (!datas.WriteString(policy)) {
        return;
    }

    uint32_t code = static_cast<uint32_t>(IDlpPermissionServiceIpcCode::COMMAND_SET_ENTERPRISE_POLICY);
    MessageParcel reply;
...
bool SetEnterprisePolicyFuzzTest(const uint8_t* data, size_t size)
{
    FuzzTest(data, size);
    return true;
}
...
{
    OHOS::SetEnterprisePolicyFuzzTest(data, size);
    return 0;
}
