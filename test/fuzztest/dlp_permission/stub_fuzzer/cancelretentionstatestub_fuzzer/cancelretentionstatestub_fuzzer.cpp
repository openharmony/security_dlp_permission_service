...
#include "cancelretentionstatestub_fuzzer.h"
#include <iostream>
#include <string>
#include <thread>
#include <vector>
...
static void FuzzTest(const uint8_t* data, size_t size)
{
    ...
    if (!datas.WriteStringVector(docUriVec)) {
        return;
    }
    uint32_t code = static_cast<uint32_t>(IDlpPermissionServiceIpcCode::COMMAND_CANCEL_RETENTION_STATE);
    MessageParcel reply;
...
bool CancelRetentionStateFuzzTest(const uint8_t* data, size_t size)
{
    FuzzTest(data, size);
    return true;
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::CancelRetentionStateFuzzTest(data, size);
    return 0;
}
