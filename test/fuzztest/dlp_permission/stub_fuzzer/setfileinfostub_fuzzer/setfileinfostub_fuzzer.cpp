...
#include "setfileinfostub_fuzzer.h"
...
{
    if ((data == nullptr) || (size == 0) || (size > MAX_SAFE_SIZE)) {
        return;
    }
    FuzzedDataProvider fdp(data, size);
    std::string uri = fdp.ConsumeBytesAsString(size / 5);
    std::string accountName = fdp.ConsumeBytesAsString(size / 5);
    std::string maskInfo = fdp.ConsumeBytesAsString(size / 5);
    std::string fileId = fdp.ConsumeBytesAsString(size / 5);
    bool isNotOwnerAndReadOnce = fdp.ConsumeBool();
    bool isWatermark = fdp.ConsumeBool();

    FileInfo fileInfo;
    fileInfo.isNotOwnerAndReadOnce = isNotOwnerAndReadOnce;
    fileInfo.isWatermark = isWatermark;
    fileInfo.accountName = accountName;
    fileInfo.maskInfo = maskInfo;
    ...
    if (!fileInfo.Marshalling(datas)) {
        return;
    }

    uint32_t code = static_cast<uint32_t>(IDlpPermissionServiceIpcCode::COMMAND_SET_FILE_INFO);
    MessageParcel reply;
...
bool SetFileInfoFuzzTest(const uint8_t* data, size_t size)
{
    FuzzTest(data, size);
    return true;
}
...
{
    OHOS::SetFileInfoFuzzTest(data, size);
    return 0;
}
