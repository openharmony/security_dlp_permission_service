...
#include "getdlpsupportfiletypestub_fuzzer.h"
#include <iostream>
#include <string>
#include <vector>
#include <thread>
...
static void FuzzTest(const uint8_t* data, size_t size)
{
    ...
    uint32_t code = static_cast<uint32_t>(IDlpPermissionServiceIpcCode::COMMAND_GET_DLP_SUPPORT_FILE_TYPE);
    MessageParcel reply;
...
bool GetDlpSupportFileTypeFuzzTest(const uint8_t* data, size_t size)
{
    FuzzTest(data, size);
    return true;
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::GetDlpSupportFileTypeFuzzTest(data, size);
    return 0;
}
