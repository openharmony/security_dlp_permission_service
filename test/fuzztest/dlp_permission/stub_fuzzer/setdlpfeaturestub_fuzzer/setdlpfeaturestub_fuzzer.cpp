...
#include "setdlpfeaturestub_fuzzer.h"
...
{
    if ((data == nullptr) || (size < sizeof(uint32_t)) || (size > MAX_SAFE_SIZE)) {
        return;
    }
    FuzzedDataProvider fdp(data, size);
    uint32_t dlpFeatureInfo = fdp.ConsumeIntegral<uint32_t>();

    MessageParcel datas;
    datas.WriteInterfaceToken(IDlpPermissionService::GetDescriptor());
    if (!datas.WriteUint32(dlpFeatureInfo)) {
        return;
    }

    uint32_t code = static_cast<uint32_t>(IDlpPermissionServiceIpcCode::COMMAND_SET_DLP_FEATURE);
    MessageParcel reply;
