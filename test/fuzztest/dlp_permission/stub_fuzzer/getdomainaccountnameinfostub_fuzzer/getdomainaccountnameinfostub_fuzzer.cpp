...
#include "getdomainaccountnameinfostub_fuzzer.h"
...
static void FuzzTest(const uint8_t* data, size_t size)
{
    ...
    uint32_t code = static_cast<uint32_t>(IDlpPermissionServiceIpcCode::COMMAND_GET_DOMAIN_ACCOUNT_NAME_INFO);
    MessageParcel reply;
    ...
    std::string accountNameInfo = fdp.ConsumeBytesAsString(size);
    service->GetDomainAccountNameInfo(accountNameInfo);
}

bool GetDomainAccountNameInfoFuzzTest(const uint8_t* data, size_t size)
{
    FuzzTest(data, size);
    return true;
}
...
{
    OHOS::GetDomainAccountNameInfoFuzzTest(data, size);
    return 0;
}
