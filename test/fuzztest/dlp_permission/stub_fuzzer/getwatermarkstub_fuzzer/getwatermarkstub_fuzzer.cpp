...
#include "getwatermarkstub_fuzzer.h"
...
{
    if ((data == nullptr) || (size < sizeof(uint8_t)) || (size > MAX_SAFE_SIZE)) {
        return;
    }
    ...
    if (!datas.WriteBool(waterMarkConfig)) {
        return;
    }

    uint32_t code = static_cast<uint32_t>(IDlpPermissionServiceIpcCode::COMMAND_GET_WATER_MARK);
    MessageParcel reply;
    ...
    sptr<DlpFuzzRemoteObj> callbackObj = new (std::nothrow) IRemoteStub<DlpFuzzRemoteObj>();
    sptr<IDlpPermissionCallback> callback = iface_cast<IDlpPermissionCallback>(callbackObj->AsObject());
    service->GetWaterMark(waterMarkConfig, callback);
}

bool GetWaterMarkFuzzTest(const uint8_t* data, size_t size)
{
    FuzzTest(data, size);
    return true;
}
...
{
    OHOS::GetWaterMarkFuzzTest(data, size);
    return 0;
}
