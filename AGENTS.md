...
系统适配：仅 `standard`（标准系统），由 `bundle.json` 的 `adapted_system_type` 声明。

### 按任务类型定位代码

| 任务类型 | 首选目录 | 关键文件 |
| --- | --- | --- |
| 修改 NAPI JS API（dlpPermission / dlpSetDlpFeature） | `interfaces/kits/dlp_permission/napi/` | `include/napi_dlp_permission.h`、`napi_dlp_feature.h`、`napi_dlp_connection_plugin.h`、`napi_dlp_transparent_enc.h`、`dlp_transparent_enc_manager.h`、`src/*.cpp` |
| 修改 NAPI JS API（敏感内容识别） | `interfaces/kits/identify_sensitive_content/napi/` | `include/`、`src/`（dlopen 外部 DIA 能力库） |
...
| 修改账号适配 | `services/dlp_permission/sa/adapt_utils/account_adapt/` | 账号事件订阅、状态监听 |
| 修改算法/HUKS 适配 | `services/dlp_permission/sa/adapt_utils/alg_adapt/` | `alg_manager/`（算法管理）、`huks_adapt_manager/`（HUKS 适配——本仓通过此层调用 HUKS，不直接使用密钥） |
...
| 修改沙箱变更回调 | `services/dlp_permission/sa/callback/dlp_sandbox_change_callback/` | `dlp_sandbox_change_callback_manager.*`、`dlp_sandbox_change_callback_proxy.*`、`dlp_sandbox_change_callback_death_recipient.*` |
| 修改打开 DLP 文件回调 | `services/dlp_permission/sa/callback/open_dlp_file_callback/` | `open_dlp_file_callback_manager.*`、`open_dlp_file_callback_proxy.*`、`open_dlp_file_callback_death_recipient.*` |
| 修改 KV 持久化 | `services/dlp_permission/sa/storage/` | `include/dlp_kv_data_storage.h`、`sandbox_config_kv_data_storage.h`、`src/*.cpp` |
| 修改 SA 注册（SA ID/进程/库路径） | `services/dlp_permission/sa/sa_profile/` | `3521.json`（SA ID 3521，进程 `dlp_permission_service`，库 `libdlp_permission_service.z.so`） |
...
| 修改 IPC 接口码（需评审） | `frameworks/dlp_permission/include/` | `dlp_permission_service_ipc_interface_code.h`（见项目约束） |
...
| 修改 DFX 事件 | `hisysevent.yaml`、`hisysevent-DLP_UE.yaml` | domain `DLP`（FAULT/CRITICAL）、`DLP_UE`（BEHAVIOR/MINOR，preserve: true） |
| 修改单元测试 | `test/unittest/dlp_permission/`、`test/unittest/dlp_parse/`、`test/unittest/dlp_fuse/`、`test/unittest/sa/` | 见"关键测试目标" |
| 修改 fuzz 目标 | `test/fuzztest/dlp_permission/fuzzer/` | 40+ fuzzer 目标，按服务方法/Stub 分组 |
...
构建基于 GN，从 OpenHarmony 源码根目录执行，**不在本子目录执行**。本仓仅支持 `standard` 系统标准产品（如 `rk3568`）。
...
2. **本地构建通过** - 至少 `dlp_permission_build_module` 构建通过
...
7. **板侧/真机验证（如适用）** - 涉及沙箱拉起、DLP 文件打开、保留回收、HUKS 集成的改动需提供板侧证据
...
报告应包含：改动摘要（文件列表、改动点）、验证结果（构建/测试输出）、影响评估（公共 API、IPC、DLP 文件格式、权限语义、DFX 事件兼容性）、风险评估（安全边界、跨账号隔离、密钥材料处理、性能风险）、未完成事项。
...
| 场景 | 修改目录 | 先读 |
| --- | --- | --- |
| 生成 DLP 文件（证书签发链路） | `sa/sa_main/` | `dlp_permission_service.h`（GenerateDlpCertificate）→ `dlp_credential.h`（与外部凭证服务通信）→ `frameworks/common/include/cert_parcel.h` |
| 打开 DLP 文件 / 拉起沙箱 | `sa/sa_main/` | `dlp_permission_service.h`（ParseDlpCertificate / InstallDlpSandbox / UninstallDlpSandbox / IsInDlpSandbox） |
| 沙箱生命周期（安装/卸载/保留/回收） | `sa/sa_main/`、`adapt_utils/file_manager/` | `dlp_permission_service.cpp` 沙箱相关方法 + `retention_file_manager.h` + `sandbox_json_manager.h` + `frameworks/common/include/retention_sandbox_info.h` |
| 权限策略 / 档位 / 动作位 | `frameworks/common/`、`inner_api/dlp_permission/` | `frameworks/common/include/permission_policy.h` + `interfaces/inner_api/dlp_permission/DlpPermissionTypes.idl`（`DLPFileAccess` 枚举） |
| 访问记录 | `adapt_utils/file_manager/` | `visit_record_file_manager.h` + `visit_record_json_manager.h` + `frameworks/common/include/visited_dlp_file_info.h` |
| DLP 文件格式/加解密 | `inner_api/dlp_parse/` | `dlp_file.h`（头/证书/密文区布局）、`dlp_crypt.h`（AES/HMAC）、`dlp_raw_file.h`（raw 文件头解析）、`dlp_zip_file.h`（zip 容器）、`dlp_file_manager.h`、`dlp_file_kits.h` |
| 沙箱文件视图 / FUSE | `inner_api/dlp_fuse/` | `dlp_link_file.h`、`dlp_link_manager.h`、`fuse_daemon.h`、`dlp_fuse_helper.h`、`dlp_fuse_fd.h` |
| KV 持久化 / 沙箱配置 | `sa/storage/` | `dlp_kv_data_storage.h`、`sandbox_config_kv_data_storage.h` |
| 修改 IPC 接口或消息体 | `inner_api/dlp_permission/` | `IDlpPermissionService.idl`（生成源，禁止手改生成产物）→ 客户端 `src/` → 服务侧 `dlp_permission_async_stub` / `sa_main`；接口码在 `frameworks/dlp_permission/include/dlp_permission_service_ipc_interface_code.h` |
| 回调机制 | `sa/callback/`、`inner_api/dlp_permission/include/` | `dlp_sandbox_change_callback*`、`open_dlp_file_callback*`；服务侧 manager + proxy + death_recipient |
| 敏感内容识别（DIA） | `kits/identify_sensitive_content/napi/` | napi 内 dlopen DIA 能力库；`identify_sensitive_content.gni` 中 `data_identify_anonymize_service_enable` |
| MDM / 企业策略 | `sa/sa_main/` | `dlp_credential.h`（SetMDMPolicy / GetMDMPolicy / RemoveMDMPolicy / SetEnterprisePolicy / SetEnterpriseInfos / QueryOpenedEnterpriseDlpFiles / CloseOpenedEnterpriseDlpFiles） |
| 透明加密 | `kits/dlp_permission/napi/`、`inner_api/dlp_parse/` | `dlp_transparent_enc_manager.h`、`napi_dlp_transparent_enc.h`、`dlp_transparent_enc_policy.h` |
| 与外部凭证服务交互 | `sa/sa_main/`、`sa/mock/` | `dlp_credential.h/.cpp`（生产客户端）；测试用 `sa/mock/` 桩，禁止把 mock 当线上实现 |
| HUKS 适配 | `sa/adapt_utils/alg_adapt/huks_adapt_manager/` | 本仓通过此层调用 HUKS，不直接使用密钥；改此层前先读 HUKS 仓的对应接口契约 |
| 公共 API / NAPI / C API | `interfaces/kits/` | `dlp_permission_api.h`、`napi_dlp_*.h`；改动需兼容性评估，NAPI 与 C 实现同步 |
| DFX / 日志 / 事件 | 任意 DFX 改动 | `hisysevent.yaml`（domain `DLP`）、`hisysevent-DLP_UE.yaml`（domain `DLP_UE`）、`frameworks/common/include/dlp_dfx_define.h`、`dlp_permission_log.h` |
| 特性开关 | `dlp_permission_service.gni`、`identify_sensitive_content.gni`、`bundle.json` | 见"构建开关"表 |
| SA 注册 | `sa/sa_profile/` | `3521.json`（SA ID/进程/库路径） |
| 构建配置 | `BUILD.gn`、`bundle.json`、`config/` | — |
| 水印 | `sa/sa_main/`、`sa_common/` | `dlp_permission_service.h`（SetWaterMark / GetWaterMark）、`sa_common/water_mark_info.h` |
| 配置文件（支持文件类型白名单） | `sa/etc/` | `dlp_config.json`（`support_file_type` 数组）、`dlp_permission.para`（gathering 策略）、`dlp_permission.para.dac`（DAC） |
...
| 术语 | 含义与去向 |
| --- | --- |
| DLP 文件 | 含头区/证书区/密文区的容器式权限文件，magic、分区块偏移与版本字段是全仓统一理解的文件格式。读 `interfaces/inner_api/dlp_parse/include/dlp_file.h` |
| DLP 凭证 / DLP 证书 / credential | 由外部 DLP 凭证服务签发、封装文件密钥与权限策略的证书，本仓是消费方。读 `sa/sa_main/dlp_credential.h` + `frameworks/common/include/cert_parcel.h` |
| 权限策略 / Policy / 策略透传 | 随 DLP 文件/证书携带的 per-file 权限声明（owner/授权账号/everyonePerm + ActionFlags），打开时解析出策略再据此拉起沙箱。读 `frameworks/common/include/permission_policy.h` |
| DLPFileAccess 档位 | DLP 权限档位：`NO_PERMISSION=0` < `READ_ONLY=1` < `CONTENT_EDIT=2` < `FULL_CONTROL=3`，判定以策略为上限。读 `interfaces/inner_api/dlp_permission/DlpPermissionTypes.idl` |
| ActionFlags / 动作位 | DLP 策略对保存/另存/编辑/截屏/录屏/复制/打印/导出等动作的逐项开关。读 `frameworks/common/include/permission_policy.h` |
| DLP 沙箱 / sandbox | 打开 DLP 文件时按 bundleName + appIndex + userId 三元组安装的受限应用实例，DLP 权限在此承载。读 `sa/sa_common/dlp_sandbox_info.h` + `sa/sa_main/dlp_permission_service.h` |
| 沙箱文件（dlp_fuse / link） | 沙箱内访问 DLP 文件内容的用户态 FUSE/link 支撑。读 `interfaces/inner_api/dlp_fuse/include/`（先读头文件弄清与沙箱文件访问路径的约定） |
| Retention / 沙箱保留 | DLP 沙箱在应用退出后按 docUri 保留、可恢复的机制。读 `sa/adapt_utils/file_manager/retention_file_manager.h` |
| 访问记录 / visit record | DLP 文件被访问/采集的记录体系，受 gathering 策略约束。读 `sa/adapt_utils/file_manager/visit_record_file_manager.h` |
| Gathering / 采集策略 | 决定是否采集 DLP 使用信息的上报策略（SA 侧 GetDlpGatheringPolicy）。读 `dlp_permission_service.gni` 中 `dlp_permission_service_gathering_policy`、`sa/etc/dlp_permission.para` |
| MDM / 企业策略 | 企业场景下对 DLP 应用名单/策略的管控入口。读 `sa/sa_main/dlp_credential.h`（SetMDMPolicy / CheckMdmPermission / SetEnterprisePolicy） |
| SetDlpFeature / dlpSetDlpFeature | "是否具备 DLP 能力/是否启用"的系统级特性位设置。读 `interfaces/kits/dlp_permission/napi/include/napi_dlp_feature.h` |
| DIA / 敏感内容识别 | 面向 DLP 文件的敏感内容识别与匿名化联动（dlopen 外部能力库）。读 `interfaces/kits/identify_sensitive_content/napi/`；开关 `data_identify_anonymize_service_enable` |
| AuthType / 认证类型 | DLP 策略中在线/离线认证的判定维度，涉及离线证书场景。读 `frameworks/common/include/permission_policy.h` |
| HiSysEvent domain DLP / DLP_UE | 两个事件域分别对应服务故障（FAULT/CRITICAL）与用户行为（BEHAVIOR/MINOR，preserve: true），定义是硬性合约。读 `hisysevent.yaml`、`hisysevent-DLP_UE.yaml` |
| IDL / Proxy / Stub | `idl_gen_interface`（target `dlp_permission_interface`）生成产物；改 `.idl` 后重新生成，禁止手改。读 `interfaces/inner_api/dlp_permission/BUILD.gn`、`IDlpPermissionService.idl`、`DlpPermissionTypes.idl` |
| 透明加密 | 不显式生成 DLP 文件、对应用透明的加密机制。读 `dlp_transparent_enc_policy.h`、`napi_dlp_transparent_enc.h`、`dlp_transparent_enc_manager.h` |
| 水印 / WaterMark | DLP 文件被打开时叠加的水印。读 `sa/sa_main/dlp_permission_service.h`（SetWaterMark / GetWaterMark）、`sa_common/water_mark_info.h` |
| 支持文件类型白名单 | DLP 允许保护的文件后缀列表。读 `sa/etc/dlp_config.json` 中 `support_file_type` |
| SA 3521 | 本服务的 SystemAbility ID。读 `sa/sa_profile/3521.json`（进程 `dlp_permission_service`，库 `libdlp_permission_service.z.so`） |

### 官方开发指南路由

应用层 NAPI/C API 行为、DLP 概念与权限档位语义以官方《数据防泄漏（DLP）开发指南》为准，改动 `interfaces/kits/*` 或权限档位语义前必先对照对应页。

基线目录：`https://gitcode.com/openharmony/docs/blob/master/zh-cn/application-dev/security/DataLossPrevention/`（目录下各 `dlp-*.md`，索引见 `Readme-CN.md`）

| 改动主题 | 先读页（相对基线目录） |
| --- | --- |
| DLP 概述与架构 | `dlp-overview.md` |
| DLP 文件生成 | `dlp-file-generation.md` |
| DLP 文件打开与沙箱 | `dlp-file-opening.md` |
| DLP 访问控制与权限档位 | `dlp-access-control.md` |
| 沙箱应用开发 | `dlp-sandbox-application-development.md` |
| 敏感内容识别（如启用 DIA） | `dlp-sensitive-content-identification.md` |

> 若上述页路径在 docs 仓中已调整，以 `DataLossPrevention/Readme-CN.md` 索引为准。
...
- C/C++ 改动优先复用项目内已有的返回约定与宏（如 `DLP_OK`、`DLP_ERROR` 系列错误码、`DLP_LOG*` 宏），不要引入风格不一致的错误处理
- 优先使用项目已有的日志宏（`frameworks/common/include/dlp_permission_log.h`），不要混用 `printf`
- 错误码使用 `frameworks/common/include/dlp_permission.h` 中已定义值，不要新增冗余错误码除非确认现有无法覆盖
- 密钥材料、证书内容等敏感信息不得写入非安全日志或 HiSysEvent

### 公共 API 约束

**Do not（禁止）：**
- 修改已发布的 NAPI JS API（`interfaces/kits/dlp_permission/napi/`、`interfaces/kits/identify_sensitive_content/napi/`）的签名、参数类型、返回值类型
- 修改已发布的 C API（`interfaces/kits/c/include/dlp_permission_api.h`）的签名
...
- 修改 `dlp_permission.para.dac` 中的 DAC 权限而不经安全评审
- 关闭或弱化 NAPI 层的系统应用校验（如 `GenerateDlpFileForEnterprise` / `DecryptDlpFile` / `QueryDlpPolicy` 等）
...
- 修改 `frameworks/dlp_permission/include/dlp_permission_service_ipc_interface_code.h` 中的接口码值/顺序
...
**Ask before（修改前必须确认）：**
- 新增 IPC 接口码：需在 `dlp_permission_service_ipc_interface_code.h` 新增，并确认跨版本兼容
...
- 涉及沙箱拉起、DLP 文件打开、保留回收的改动需在真机/模拟器验证，提供板侧证据（日志、HiSysEvent 输出）
...
2. **IPC 接口码 / 序列化顺序 / `dlp_permission_service_ipc_interface_code.h`** → 跨进程兼容破坏
